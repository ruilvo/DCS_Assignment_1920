function berout = modem_awgn(modemtype, M, bitsin, tsnrs, varargin)
    % MODEM_AWGN Do the modem stuff
    % varargins  fs, sps, rcbeta, rcspan, fsep, FECtype, crcCoding, trellisArgs
    ...
    berout = zeros(length(tsnrs), 1);

    for a = 1:length(tsnrs)
        tsnr = tsnrs(a);

        fomatSpec = "SNR = %d; %s, M = %d\n";
